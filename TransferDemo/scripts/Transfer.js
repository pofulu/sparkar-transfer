...
const SCREEN_POSITION_KEYWORD = 'pp';
...
targets.then(results => {
    ...
    const setAllFromScript = results.map(target => {
        ...
        return Promise.all([
            ...
            Patches.inputs.setPoint(pattern.replace('KEY', SCREEN_POSITION_KEYWORD), Reactive.pack3(getPercX(target.transform.x), getPercY(target.transform.y), 0)),
